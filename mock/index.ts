...
import { permissionList, roleList, userList } from "./data";
...
const mockList: MockMethod[] = [
    ...
    },
    {
        ...
        statusCode: 200,
        response: () => {
            const arrRoleList = roleList.forEach((role: any) => {
                let permissionNames: any[] = [];
                for (const i in role.permission) {
                    permissionNames.push(
                        find(permissionList, ["id", role.permission[i]]).name
                    );
                }
    ...
    },
    {
        ...
        response: ({ query }) => {
            ...
            // 获取用户权限
            const permissionIdS = find(roleList, [
                "id",
                user.role.roleId,
            ]).permission;
            // 获取权限列表
            ...
            return {
                ...
                result: Object.assign(user, {
                    permissionList: listToTree(pList),
                }),
            };
        },
    },
